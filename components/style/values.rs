...
pub mod specified {
    ...
    impl CSSColor {
        pub fn parse(input: &mut Parser) -> Result<CSSColor, ()> {
            let authored = match input.peek() {
                Ok(&Token::Ident(ref s)) => Some(s.clone().into_owned()),
                _ => None,
            };
            Ok(CSSColor {
    ...
    fn parse_one_color_stop(input: &mut Parser) -> Result<ColorStop, ()> {
        Ok(ColorStop {
            ...
            position: LengthOrPercentage::parse(input).ok(),
        })
    }
    ...
    impl LinearGradient {
        ...
        pub fn parse_function(input: &mut Parser) -> Result<LinearGradient, ()> {
            let angle_or_corner = if let Ok(angle) = Angle::parse(input) {
                AngleOrCorner::Angle(angle)
            } else {
                try!(input.expect_ident_matching("to"));
                let (horizontal, vertical) =
                if let Ok(value) = HorizontalDirection::parse(input) {
                    (Some(value), VerticalDirection::parse(input).ok())
                } else if let Ok(value) = VerticalDirection::parse(input) {
                    (HorizontalDirection::parse(input).ok(), Some(value))
                } else {
                    return Err(())
                };
    ...
    pub fn parse_border_width(input: &mut Parser) -> Result<Length, ()> {
        if let Ok(value) = input.expect_ident() {
            match_ignore_ascii_case! { value:
                "thin" => return Ok(Length::from_px(1.)),
                "medium" => return Ok(Length::from_px(3.)),
                "thick" => return Ok(Length::from_px(5.))
                _ => {}
            }
