...
use cssparser::{Parser, Color, RGBA, AtRuleParser, DeclarationParser,
                DeclarationListParser, parse_important, Priority};
...
//            }
    ...
    <%self:longhand name="z-index">
        ...
        fn parse(_context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue, ()> {
            if input.expect_ident_matching("auto").is_ok() {
    ...
    <%self:longhand name="vertical-align">
        ...
        /// | <percentage> | <length>
        pub fn parse(_context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue, ()> {
            if let Ok(value) = specified::LengthOrPercentage::parse_non_negative(input) {
                Ok(SpecifiedValue::LengthOrPercentage(value))
            } else {
                let value = try!(input.expect_ident());
                match_ignore_ascii_case! { value:
                    % for keyword in vertical_align_keywords[:-1]:
                        "${keyword}" => return Ok(SpecifiedValue::${to_rust_ident(keyword)}),
                    % endfor
    ...
    <%self:longhand name="content">
            ...
            pub fn parse(_context: &ParserContext, input: &mut Parser)
                         -> Result<SpecifiedValue, ()> {
                if input.expect_ident_matching("normal").is_ok() {
    ...
    <%self:longhand name="background-position">
            ...
            pub fn parse(_context: &ParserContext, input: &mut Parser)
                ...
                let second = specified::PositionComponent::parse(input)
...
//                Monospace,
        ...
        pub fn parse_one_family(input: &mut Parser) -> Result<FontFamily, ()> {
            if let Ok(value) = input.expect_string() {
...
//            }
    ...
    <%self:longhand name="font-weight">
        ...
        pub fn parse(_context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue, ()> {
            if let Ok(value) = input.expect_ident() {
                match_ignore_ascii_case! { value:
                    "bold" => return Ok(SpecifiedValue::Weight700),
                    "normal" => return Ok(SpecifiedValue::Weight400),
                    "bolder" => return Ok(SpecifiedValue::Bolder),
                    "lighter" => return Ok(SpecifiedValue::Lighter)
                    _ => {}
                }
                input.push_back(Token::Ident(value))
            }
    ...
    <%self:longhand name="font-size">
        ...
        pub fn parse(_context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue, ()> {
            match specified::LengthOrPercentage::parse_non_negative(input) {
                ...
                Ok(specified::LengthOrPercentage::Percentage(value)) => {
                    return Ok(specified::Length::Em(value))
                }
                Err(()) => (),
            }
            let value = try!(input.expect_ident());
            match_ignore_ascii_case! { value:
                "xx-small" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX) * 3 / 5)),
                "x-small" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX) * 3 / 4)),
                "small" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX) * 8 / 9)),
                "medium" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX))),
                "large" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX) * 6 / 5)),
                "x-large" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX) * 3 / 2)),
                "xx-large" => return Ok(specified::Length::Au(Au::from_px(MEDIUM_PX) * 2)),

                // https://github.com/servo/servo/issues/3423#issuecomment-56321664
                "smaller" => return Ok(specified::Length::Em(0.85)),
                "larger" => return Ok(specified::Length::Em(1.2))
    ...
    <%self:longhand name="box-shadow">
        ...
        pub fn parse_one_box_shadow(input: &mut Parser) -> Result<SpecifiedBoxShadow, ()> {
            ...
            loop {
                if !inset {
                    if input.expect_ident_matching("inset").is_ok() {
                        inset = true;
                        continue
                    }
                }
                if !lengths_parsed {
                    if let Ok(value) = specified::Length::parse(input) {
                        lengths[0] = value;
                ...
                if color.is_none() {
                    if let Ok(value) = specified::CSSColor::parse(input) {
                        color = Some(value);
                        continue
                    }
                }
    ...
    <%self:longhand name="filter">
        ...
        pub fn parse(_context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue, ()> {
            ...
            loop {
                if let Ok(function_name) = input.expect_function() {
                    filters.push(try!(input.parse_nested_block(|input| {
                        match_ignore_ascii_case! { function_name:
                            "brightness" => input.expect_percentage().map(Filter::Brightness),
                            "contrast" => input.expect_percentage().map(Filter::Contrast),
                            "grayscale" => input.expect_percentage().map(Filter::Grayscale),
                            "hue-rotate" => Angle::parse(input).map(Filter::HueRotate),
                            "invert" => input.expect_percentage().map(Filter::Invert),
                            "opacity" => input.expect_percentage().map(Filter::Opacity),
                            "saturate" => input.expect_percentage().map(Filter::Saturate),
                            "sepia" => input.expect_percentage().map(Filter::Sepia)
                            _ => Err(())
                        }
                    })));
                } else {
...
pub mod shorthands {
    ...
    <%def name="four_sides_shorthand(name, sub_property_pattern, parser_function)">
        <%self:shorthand name="${name}" sub_properties="${
            ...
            let right = ${parser_function}(input).unwrap_or_else(|()| top.clone());
            let bottom = ${parser_function}(input).unwrap_or_else(|()| top.clone());
            let left = ${parser_function}(input).unwrap_or_else(|()| right.clone());
            Ok(Longhands {
                % for side in ["top", "right", "bottom", "left"]:
    ...
    <%self:shorthand name="background"
        ...
        loop {
            if position.is_none() {
                if let Ok(value) = background_position::parse(context, input) {
                    position = Some(value);
                    any = true;
                    continue
                }
            ...
            if color.is_none() {
                if let Ok(value) = background_color::parse(context, input) {
                    color = Some(value);
                    any = true;
                    continue
                }
            ...
            if image.is_none() {
                if let Ok(value) = background_image::parse(context, input) {
                    image = Some(value);
                    any = true;
                    continue
                }
            ...
            if repeat.is_none() {
                if let Ok(value) = background_repeat::parse(context, input) {
                    repeat = Some(value);
                    any = true;
                    continue
                }
            ...
            if attachment.is_none() {
                if let Ok(value) = background_attachment::parse(context, input) {
    ...
    pub fn parse_border(context: &ParserContext, input: &mut Parser)
        ...
        loop {
            ...
            if style.is_none() {
                if let Ok(value) = specified::BorderStyle::parse(input) {
                    style = Some(value);
                    any = true;
                    continue
                }
            ...
            if width.is_none() {
                if let Ok(value) = specified::parse_border_width(input) {
                    width = Some(value);
                    any = true;
                    continue
                }
    ...
    <%self:shorthand name="font" sub_properties="font-style font-variant font-weight
        ...
        loop {
            ...
            if style.is_none() {
                if let Ok(value) = font_style::parse(context, input) {
                    style = Some(value);
                    continue
                }
            }
            if weight.is_none() {
                if let Ok(value) = font_weight::parse(context, input) {
                    weight = Some(value);
                    continue
                }
            }
            if variant.is_none() {
                if let Ok(value) = font_variant::parse(context, input) {
                    variant = Some(value);
                    continue
                }
            }
    ...
    <%self:shorthand name="list-style"
        ...
        loop {
            ...
            if list_style_type.is_none() {
                if let Ok(value) = list_style_type::parse(context, input) {
            ...
            if image.is_none() {
                if let Ok(value) = list_style_image::parse(context, input) {
                    image = Some(value);
                    any = true;
                    continue
                }
            ...
            if position.is_none() {
                if let Ok(value) = list_style_position::parse(context, input) {
                    position = Some(value);
                    any = true;
                    continue
                }
...
impl<'a, 'b> DeclarationParser<()> for PropertyDeclarationParser<'a, 'b> {
    fn parse_value(&mut self, name: &str, input: &mut Parser) -> Result<(), ()> {
        ...
        match try!(parse_important(input)) {
            Priority::Normal => &mut self.normal_declarations,
            Priority::Important => &mut self.important_declarations,
        }.push_all(results.as_slice());
        Ok(())
    }
}
...
impl CSSWideKeyword {
    pub fn parse(input: &mut Parser) -> Result<CSSWideKeyword, ()> {
        let value = try!(input.expect_ident());
        match_ignore_ascii_case! { value:
            "initial" => return Ok(CSSWideKeyword::InitialKeyword),
            "inherit" => return Ok(CSSWideKeyword::InheritKeyword),
            "unset" => return Ok(CSSWideKeyword::UnsetKeyword)
            _ => {}
        }
        input.unexpected_ident(value)
    }
