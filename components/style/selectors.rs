...
fn parse_attribute_selector(context: &ParserContext, input: &mut Parser)
    ...
    macro_rules! parse_value {
        () => {
            try!(input.expect_ident_or_string()).into_owned()
        }
    }
    ...
    match input.next() {
        ...
        // [foo=bar]
        Ok(Token::Delim('=')) => Ok(SimpleSelector::AttrEqual(
            attr, parse_value!(), try!(parse_attribute_flags(input)))),

        // [foo~=bar]
        Ok(Token::IncludeMatch) => Ok(SimpleSelector::AttrIncludes(attr, parse_value!())),
        ...
        // [foo^=bar]
        Ok(Token::PrefixMatch) => Ok(SimpleSelector::AttrPrefixMatch(attr, parse_value!())),

        // [foo*=bar]
        Ok(Token::SubstringMatch) => Ok(SimpleSelector::AttrSubstringMatch(attr, parse_value!())),

        // [foo$=bar]
        Ok(Token::SuffixMatch) => Ok(SimpleSelector::AttrSuffixMatch(attr, parse_value!())),
