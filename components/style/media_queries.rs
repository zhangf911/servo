...
impl Expression {
    fn parse(input: &mut Parser) -> Result<Expression, ()> {
        ...
        match try!(input.next()) {
            ...
            token => return input.unexpected(token)
        }
...
impl MediaQuery {
    fn parse(input: &mut Parser) -> Result<MediaQuery, ()> {
        ...
        let qualifier = if input.expect_ident_matching("only").is_ok() {
            Some(Qualifier::Only)
        } else if input.expect_ident_matching("not").is_ok() {
            Some(Qualifier::Not)
        } else {
            None
        };
        ...
        loop {
            if input.expect_ident_matching("and").is_err() {
